...
import stu.thetinkerj.kg.queryRewriting.axiomsReplace.AxiomCollect;
import stu.thetinkerj.kg.queryRewriting.conceptExtract.Concept;
import stu.thetinkerj.kg.queryRewriting.conceptExtract.ConceptUtilNodeWrapper;
import stu.thetinkerj.kg.queryRewriting.tripleUtils.OpUtil;
import stu.thetinkerj.kg.queryRewriting.tripleUtils.TripleUtil;
...
//    private final OpProject op;
    ...
    public AlgebraTransformer(Concept concept,Boolean extractFlag, int limit) throws OWLOntologyCreationException, OWLOntologyStorageException, FileNotFoundException {
        stack = new Stack<Op>();
        concepts = concept;
   ...
   // triple= SubAxiomReplace.replaceAxiomVar(triple);
    ...
    //
    public Op BothTransform2(Op op){

        op= new AxiomCollect().AxiomEasyReplaceOpproject(op,concepts.getModel());
        return   transform(op);
    }
